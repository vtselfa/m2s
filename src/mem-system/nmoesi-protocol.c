...
void mod_handler_pref(int event, void *data)
{
	...
	if (event == EV_MOD_PREF_MISS)
	{
		...
		if(stack->prefetch_hit)
			cache_set_pref_block(cache, stack->src_pref_stream, stack->src_pref_slot, 0, cache_block_invalid); //SLOT
	...
	if (event == EV_MOD_PREF_UNLOCK)
	{
		...
		if(stack->prefetch_hit)
			dir_pref_entry_unlock(mod->dir, stack->src_pref_stream, stack->src_pref_slot); //SLOT
...
void mod_handler_nmoesi_find_and_lock(int event, void *data)
{
	...
	if (event == EV_MOD_NMOESI_FIND_AND_LOCK_PORT)
	{
		...
		/* Miss */
		if (!stack->hit) //VERIFICAR down up
		{
			/* Get victim */
