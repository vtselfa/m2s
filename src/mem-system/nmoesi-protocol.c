...
void enqueue_prefetch_group(int ctx_pid, int core, int thread, struct mod_t *mod, unsigned int base_addr, int stride)
{
	...
	/* Set the number of usable slots according to the prefetch policy */
	sb->eff_num_slots = cache->prefetch.pol_num_slots;

	/* Set the number of pending prefetches for this stream */
...
void enqueue_prefetch_on_hit(struct mod_stack_t *stack)
{
	...
	/* Update tail */
	sb->tail = (sb->tail + 1) % sb->eff_num_slots; //TAIL
...
/* 	uop->thread = stack->client_info->thread; */
	...
	if (event == EV_MOD_NMOESI_LOAD_FINISH)
	{
		...
		if (!stack->background)
		{
			...
			if(stack->stream_hit && stack->stream_head_hit)
			{
				...
				sb->head = (stack->pref_slot + 1) % sb->eff_num_slots; //HEAD
			}
	...
	if (event == EV_MOD_NMOESI_FIND_AND_LOCK_ACTION)
	{
		...
		if (!stack->hit && stack->state && stack->request_dir == mod_request_up_down)
		{
			...
			if (stack->stream_hit)
			{
				...
				/* Update head */
				if (stack->stream_head_hit)
					sb->head = (sb->head + 1) % sb->eff_num_slots; //HEAD
...
void mod_handler_nmoesi_invalidate_slot(int event, void *data)
{
	...
	if (event == EV_MOD_NMOESI_INVALIDATE_SLOT)
	{
		...
		stack->pref_slot = stack->client_info->slot;
		assert(stack->pref_stream >= 0 && stack->pref_stream < cache->prefetch.max_num_streams);
		assert(stack->pref_slot >= 0 && stack->pref_slot < cache->prefetch.streams[stack->pref_stream].eff_num_slots);
...
void mod_handler_nmoesi_request_main_memory(int event, void *data )
{
	...
	if (event == EV_MOD_NMOESI_EXAMINE_QUEUE_REQUEST)
		...
		mem_controller->regs_channel[stack->channel].state = channel_state_free;
			...
			mem_controller->t_core_acces[stack->client_info->core]+=t_acces;
			assert(stack->client_info->core>=0 && stack->client_info->thread>=0);
			if(mem_controller->queue_per_bank)
	...
	if (event == EV_MOD_NMOESI_ACCES_TABLE)  // the request has arrived until the rank and bank
		...
		table = mem_controller_get_row_buffer_table(mem_controller, stack->client_info->core);
                mem_debug("  %lld %lld 0x%x %s acces table\n", esim_time, stack->id, stack->addr & ~mod->cache->block_mask, mod->name);
