...
for ctxs in benchmarks/parsec3/ctxfiles/*8threads; do
  ...
  echo "Arguments = --x86-sim detailed --x86-report ${ruta_results}/${i}_cpu --main-mem-report ${ruta_results}/${i}_mm --net-report ${ruta_results}/${i}_net --mem-report ${ruta_results}/${i}_mem --net-report ${ruta_results}/${i}_net --mem-config $configuracion_cache --x86-config $configuracion_cpu --net-config $configuracion_net --ctx-config $ctxs";
